    ...
    if len(coordinates_given_by_user) == 2:
        for i in range(len(rows_letters)):
            if coordinates_given_by_user[0] == rows_letters[i]:
                row = int((i + 1))
        col = int(coordinates_given_by_user[1])
        return (row, col)
    else:
        for i in range(len(rows_letters)):
            if coordinates_given_by_user[1] == rows_letters[i]:
                row = int((i + 1))
        col = int(coordinates_given_by_user[2])
        return (row, col)
...
5: Aircraft carrier
'''
def get_placement_coordinates(ship_type_number, ship_types, board):
    checking_result = ()
    ...
    if ship_type == ship_types[0]:
        while not checking_result == 'coord_valid':      
            ...
            checking_result = check_if_coordinates_valid(coordinates_given_by_user, board, board_dimension, ship_type_number)          
...
def mark_occupied_and_forbidden_fields_on_board_in_placement_phase(board):
    pass


def mark_ship_on_board(board, coordinates_given_by_user):
    pass
...
def check_if_coordinates_valid(coordinates_given_by_user, board, board_dimension, ship_type_number):
    invalid_input = 'INVALID INPUT'
    out_of_range = 'COORDINATES OUT OF RANGE'
